...
class CheckoutController extends GetxController implements GetxService {
  ...
  double? _deliveryChargeFromAPI;
  double? get deliveryChargeFromAPI => _deliveryChargeFromAPI;
  ...
  Future<void> initCheckoutData(int? storeId) async {
    ...
    // 👇 أضيفي هذا السطر بعد السطر أعلاه
    if(_paymentMethodIndex == -1) {
      setPaymentMethod(0, isUpdate: false); // 0 = Cash on Delivery
    }
  ...
  Future<double?> getDistanceInKM(LatLng originLatLng, LatLng destinationLatLng,
    ...
    await _getDeliveryPrice();
    update();
    return _distance;
  }

  Future<void> _getDeliveryPrice() async {
    final res = await checkoutServiceInterface.getDeliveryPrice(
      store?.zoneId ?? 0,
      AddressHelper.getUserAddressFromSharedPref()!.zoneIds!.first,
      _distance!,
    );
    _deliveryChargeFromAPI = double.tryParse(res.body.toString()) ?? 0.0;
  }
  ...
  Future<String> placeOrder(
    ...
    _isLoading = true;
    String userID = AuthHelper.isLoggedIn()
    ...
    if (!isOfflinePay) {
      callback(
          response.statusCode == 200, // ← يعتمد على نجاح الاستجابة
