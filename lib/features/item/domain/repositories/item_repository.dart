...
class ItemRepository implements ItemRepositoryInterface {
  ...
  @override
  Future getList({int? offset, String? type, bool isPopularItem = false,
    bool isReviewedItem = false, bool isFeaturedCategoryItems = false,
    bool isRecommendedItems = false, bool isCommonConditions = false, bool isDiscountedItems = false,
    int? moduleId,}) async {
    if (isPopularItem) {
      return await _getPopularItemList(type!, moduleId: moduleId);
  ...
  Future<List<Item>?> _getPopularItemList(String type, {int? moduleId}) async {
    ...
    final headers = apiClient.updateHeader(
      ...
      moduleId, // ✅ استخدم القيمة الممرّرة، لا تعيد تعريفها
      address?.latitude,
      address?.longitude,
      setHeader: false,
    );
  ...
  Future<List<Item>?> _getRecommendedItemList(String type, {int? moduleId}) async {
