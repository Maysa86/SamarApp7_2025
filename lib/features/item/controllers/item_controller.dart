...
class ItemController extends GetxController implements GetxService {
  ...
  Future<void> getPopularItemList(
     bool isPopular,
      ...
      bool notify, {
        int? moduleId,
        bool reload = false,
        int limit = 20, // ✅ إضافة limit افتراضي
        int offset = 1,
      }) async {
    _popularType = type;
    ...
    if (_popularItemList == null || reload) {
      print('[DEBUG] Requesting popular items...');
      print('[DEBUG] Type: $type');
      print('[DEBUG] Module ID passed: $moduleId');

      // 🟢 استدعاء API مع التصفية
      List<Item>? items = await itemServiceInterface.getPopularItemList(
        type,
        moduleId: moduleId,
        limit: limit,
        offset: offset,
      ...
      if (items != null) {
        ...
        print('[DEBUG] Popular items count: ${items.length}');
      } else {
       print('[DEBUG] No popular items returned.');
      }
   ...
   //   );
    ...
    //  if (items != null) {
     ...
     //   _popularItemList!.addAll(items);
        ...
        // 🟢 طباعة عدد العناصر التي تم إرجاعها
     //  print('[DEBUG] Popular items count: ${items.length}');
     //   _isLoading = false;
    //  } else {
    //    print('[DEBUG] No popular items returned.');
 ...
 //}
  ...
  Future<void> getRecommendedItemList(bool reload, String type, bool notify, {int? moduleId}) async {
    ...
    if (_recommendedItemList == null || reload) {
      List<Item>? items = await itemServiceInterface.getRecommendedItemList(type, moduleId: moduleId);
